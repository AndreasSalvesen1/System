...
def hent_transaksjonerSB1():
        ...
        def forsiktig_med_429():
            ...
            print("------------------------------------------------------------------------")
            print("Sjekker i Excel om det er gått mer enn", pauseperiode_i_timer,"timer siden forrige kjøring ...")
            print("------------------------------------------------------------------------"); NL(1)
            ...
            if endringstid is not None:
                ...
                if tid_siden_endring < ventetid:
                    ...
                    gjenvaerende_tid = gjenvaerende_tid - timedelta(microseconds=gjenvaerende_tid.microseconds)
                    print("Det er", gjenvaerende_tid.seconds // 3600, "timer og", (gjenvaerende_tid.seconds % 3600) // 60, "minutter igjen til neste kjøring"); NL(1)
                    if gjenvaerende_tid > timedelta(hours=3):
                        print("Det kan kanskje være lurt å jobbe med C++ imens ..."); NL(1)
                    print("Lukker Excel-arbeidsbok slik at bruker også kan redigere i mellomtiden uten at programmet overskriver endringer ..."); NL(1)    
                    if gjenvaerende_tid > timedelta(minutes=1):
                        print("Venter til", (datetime.now() + gjenvaerende_tid).strftime("%H:%M"), "..."); NL(1)
        ...
        def hent_transaksjoner():
            ...
            print("----------------------------------------------------------------------------------------------------------------------------------")
            print("--------------------------------------- Starter henting av transaksjoner fra SpareBank1 ... --------------------------------------")
            print("----------------------------------------------------------------------------------------------------------------------------------"); NL(3)
            time.sleep(3)
            ...
            def hent_kontoer():
                ...
                request_URL = "https://api.sparebank1.no/personal/banking/accounts?" + paramatere
                reqheadere = {
                ...
                print("Ferdig med henting av kontoer etter:", round(perf_counter() - start_tid2, 3), "sekunder"); NL(1)
                # print(json.dumps(data, indent=4)); NL(1)
                print("Sparebank1 sier at kontonummere er:")
                print(kontonummere_SB1); NL(3)
            ...
            def hent_access_keys():
                ...
                response = requests.get(f"{SB1_base_url}/keys?{parametere}", headers=reqheadere)
                data = response.json()
                print("Ferdig med henting av account keys etter:", round(perf_counter() - start_tid2, 3), "sekunder"); NL(1)
                print("Account keys er:"); NL(1)
            ...
            def refresh_access_token():
                ...
                with open(REFRESH_TOKEN_FILE, "r") as file:
                    ...
                    print("Refresh token før request er:", refresh_token); NL(1)
                ...
                print(f"POST {token_adresse}"); NL(1)
                print("Parametere er:") 
                pprint.pprint(data)
                NL(1)
                ...
                print("Ferdig med oppdatering av access token etter:", round(perf_counter() - start_tid2, 3), "sekunder"); NL(1)
                #print("Respons fra SpareBank1 er:", Output_data); print()
                #print("Access token etter request er:", response_data["access_token"]); print()
                print("Refresh token etter request er:", Output_data["refresh_token"]); NL(3)
            ...
            def hent_transaksjoner(Kontonummer):
                ...
                print("----------------------------------------------------------------------------------------------------------------")
                print("Starter henting av transaksjoner fra SpareBank1 for kontonummer:", Kontonummer, f"fra: {fra} til: {til} ...")
                print("----------------------------------------------------------------------------------------------------------------"); NL(1)
                ...
                if os.path.exists(Eksisterende_CSV_fil):
                    ...
                    filtrert_data = [row for row in Eksisterende_CSV_data if row[0] not in overlappende_datoer]
                    ...
                    print(f'Oppdaterte {Eksisterende_CSV_fil} med {len(ny_data)} nye transaksjoner')
                    
                else:
                    with open(Eksisterende_CSV_fil, 'w', encoding='utf-8') as f:
                        f.write(SB1_output.text)
                    NL(1); print(f'Lagret {Eksisterende_CSV_fil} med {len(SB1_output.text.splitlines())} transaksjoner'); NL(1)

                print(f'Request for transaksjoner i kontonummer: {Kontonummer}:')
                print("-----------------------------------------------------")
                print(url, headers); NL(1)
                print("Statuskode for request er:", SB1_output.status_code)
                print("Ny refresh_token er:", open(REFRESH_TOKEN_FILE).read().strip())
                print("-----------------------------------------------------"); NL(1)
                print("Respons fra SpareBank1 er:"); NL(1)
                # print("-------------------------------------------------------------------------------------------------------")
                print(SB1_output.text); NL(1)

                if os.path.exists(Skriv_til_CSV_fil):
                    os.remove(Skriv_til_CSV_fil)
                shutil.move(Eksisterende_CSV_fil, path_til_CSV)
                print("-----------------------------------------------------------------------------------")
                print("Ferdig med henting av transaksjoner i kontonummer", Kontonummer, "etter:", round(perf_counter() - start_tid2, 3), "sekunder")
                print("-----------------------------------------------------------------------------------"); NL(3)
            ...
            def HW_test():
                ...
                SB1_output = requests.get("https://api.sparebank1.no/common/helloworld", headers=reqheadere)
                data = SB1_output.json()
                melding = data["message"]
                if melding == "Hello, World my friend!":
                    print("SB1 Hello-World test OK, access_token OK, fortsetter til neste steg ..."); NL(3)
                if melding != "Hello, World my friend!":
                    print("SB1 Hello-World test feilet, access_token feil, avslutter program ..."); NL(3)
                    exit()
            ...
            def start_funksjon():
                refresh_access_token(); time.sleep(3)
                HW_test(); time.sleep(3)
                hent_kontoer(); time.sleep(3)
                hent_access_keys(); time.sleep(3)
                hent_alle_transaksjoner(); time.sleep(3)
                # print('Alle kontoer i', hovedkatalog,'behandlet i Sparebank1 API, fortsetter nå til videre prosessering ...')

            try:
                start_funksjon()
            except Exception as e:
                print("Henting av transaksjoner feilet, grunnet manglende nett eller feil konfigurering:"); NL(1)
                # print(e); NL(1)
                print("Fortsetter programmet likevel og prøver å hente transaksjoner igjen om 8 timer ..."); NL(4)
            
            
            print("----------------------------------------------------------------------------------------------------------------------------------")
            print("-------------------------- Ferdig med henting av transaksjoner fra SpareBank1 etter:", round(perf_counter() - start_tid, 3), "sekunder -----------------------------")
            print("----------------------------------------------------------------------------------------------------------------------------------"); NL(3)

        forsiktig_med_429(); time.sleep(4)
        
        hent_transaksjoner(); time.sleep(4)
...
def last_arbeidsbok_global():
        ...
        while True:
            try:
                ...
                print("Venteperiode er satt til", round(timer, None), "time og", round(minutter, None), "minutter før neste forsøk ..."); NL(1)
                print("Venter til:", tid.strftime("%H:%M"), "for å prøve på nytt ...")
                time.sleep(venteperiode)
        NL(1)
        print("----------------------------------------------------------------")
        print("Ferdig med innlasting av Excel-arbeidsbok, etter:", round(perf_counter() - start_tid, 3), "sekunder")
        print("----------------------------------------------------------------"); NL(1)
        return arbeidsbok
...
def fjern_all_data_BtilG():
        ...
        print("Ferdig med fjerning av data fra B-F etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)


def fjern_overlappende_data_BtilG():

        print("-----------------------------------------------------------------------------------------------------")
        print("Fjerner data i kolonne B-G fra rad 9 og nedover dersom dato i kolonne B er høyere enn:", oppdater_fra_dato, "...")
        print("-----------------------------------------------------------------------------------------------------"); NL(1)
        start_tid = perf_counter()
        start_rad = 9
        slutt_rad = arbeidsark.max_row
        ...
        NL(1); print("Alle datoene i CSV-filen er:"); NL(1)
        pprint.pprint(set(csv_datoer)); NL(1)
        
        
        print("Fjerner data fra Excel hvis B-kolonne inneholder en dato som også er en av disse CSV-datoene ..."); NL(1)
        for row in range(start_rad, slutt_rad + 1):
...
def hent_neste_tomme_rad(kolonnestart, kolonneslutt, startradnummer):
        ...
        none_counter = 0
        while True:
            if all(arbeidsark.cell(row=radnummer, column=kolonnenummer).value is None for kolonnenummer in range(kolonnestart, kolonneslutt)):
                none_counter += 1
...
def hent_neste_ikke_tomme_rad(kolonnestart, kolonneslutt, startradnummer):
    ...
    not_none_counter = 0
    while True:
        if all(arbeidsark.cell(row=radnummer, column=kolonnenummer).value is not None for kolonnenummer in range(kolonnestart, kolonneslutt)):
            not_none_counter += 1
        else:
            not_none_counter = 0
...
def last_kategoriseringsregler():
        print("Starter innlasting av kategoriseringsregler fra markert seksjon ..."); NL(1)
        start_tid = perf_counter()
        ...
        for kolonne in range(9, kategoriark.max_column + 1):
            ...
            if kategori is None:
                print(f"Stopper ved kolonne: {kolonne} fordi kategori er definert som None.")
                break
        ...
        NL(1); print(f"Ferdig med innlasting av kategoriseringsregler etter: {round(perf_counter() - start_tid, 3)} sekunder"); NL(1)
...
def hent_kategoriser_og_skriv_CSV():
        ...
        for CSV_fil in CSV_filnavn:
            ...
            df['Inn'] = pd.to_numeric(df['Inn'].astype(str).str.replace(',', '.').replace('-', ''), errors='coerce').fillna(0)
            df['Ut'] = pd.to_numeric(df['Ut'].astype(str).str.replace(',', '.').replace('-', ''), errors='coerce').fillna(0)
            df['Mengde'] = df['Inn'] + df['Ut']
            df = df.drop(columns=['Inn', 'Ut'])
            # NL(5); print("Linje 467"); exit()
            df = df[~df['Beskrivelse'].astype(str).str.contains('|'.join(eksluder_disse_beskrivelser)).fillna(False)]
            ...
            df['Kategori'] = df['Beskrivelse'].apply(lambda x: kategoriser_transaksjoner(x, kategoriseringsregler))
            df = df[['Dato', 'Beskrivelse', 'Kategori', 'Mengde']]
            print("Ferdig med kategorisering etter:", round(perf_counter() - start_tid1, 3), "sekunder")

            antall_transaksjoner = 0
            print("Starter skriving av ny data tilbake til Excel fra", CSV_fil, "ved rad:", neste_tomme_rad,"...")
            ...
            print("Ferdig med skriving av ny CSV tilbake til Excel etter:", round(perf_counter() - start_write, 3), "sekunder")
            print("Ferdig med prosessering av CSV-fil", CSV_fil, "etter:", round(perf_counter() - start_tid, 3), "sekunder, med", antall_transaksjoner, "transaksjoner")
            NL(1)

        # print("----------------------------------------------------------------")
        print("Ferdig med prosessering av alle CSV-filer etter:", round(perf_counter() - start_tid2, 3), "sekunder")
        # print("----------------------------------------------------------------")
        NL(3)
...
def skriv_predefinerte_transaksjoner():
        ...
        def skriv_nye_beskrivelser():
            ...
            til1 = hent_neste_tomme_rad(23+antall_nye_kolonner, 26+antall_nye_kolonner, 3)
            print("Siste radnummer i sammenligningsdel er:", til1-1); NL(1)
            
            til2 = hent_neste_tomme_rad(2, 5, 9)
            # til2 = 20
            print("Siste radnummer i transaksjonsdel er:", til2-1); NL(1)
            ...
            for rad in range(3, til1):
                ...
                mengdeindikator = arbeidsark.cell(row=rad, column=(24+antall_nye_kolonner)).value
                indikativ_beskrivelse = arbeidsark.cell(row=rad, column=(25+antall_nye_kolonner)).value
                ny_beskrivelse = arbeidsark.cell(row=rad, column=(26+antall_nye_kolonner)).value
                # print("Ny beskrivelser er:", ny_beskrivelse)
                
                for sammenlign_rad in range(9, til2):
                    # print("Sammenlign_rad er:", sammenlign_rad)
                    sammenlign_med_beskrivelse = arbeidsark.cell(row=sammenlign_rad, column=3).value
                    sammenlign_med_mengdeindikator = arbeidsark.cell(row=sammenlign_rad, column=5).value
                    ...
                    if mengdeindikator is not None:
                        if (mengdeindikator == sammenlign_med_mengdeindikator 
                            and str(indikativ_beskrivelse) in str(sammenlign_med_beskrivelse)):
            ...
            print(f"Ferdig med omskriving av", antall_endringer,"transaksjoner i kolonne C fra rad 9 til rad", til2-1, "etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(1)
        

                        
            print("Ferdig med rekategorisering av alle transaksjoner etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)
        ...
        def rekategoriser_alle_transaksjoner():
            ...
            print("Begynner rekategoriserering fra rad 9 til rad", hent_neste_tomme_rad(2, 5, 9)-1, "... "); NL(1) 
            antall_kategoriseringer = 0
            for row in arbeidsark.iter_rows(min_row=8, min_col=3, max_col=3, max_row=hent_neste_tomme_rad(2, 5, 9)):
            ...
            print("Ferdig med rekategorisering av", antall_kategoriseringer,"transaksjoner etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)
...
def les_og_sorter():
        ...
        data = pd.DataFrame(arbeidsark.iter_rows(min_row=9, min_col=2, max_col=7, max_row=max_rad, values_only=True))
        ...
        print("Ferdig med sortering av data etter:", round(perf_counter() - start_tid, 3), "sekunder")
        ...
        print("Ferdig med konvertering av data tilbake til original form etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(1)
        ...
        NL(1); print("Ferdig med tilbakeskriving av data etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)
...
def beregn_og_skriv_saldo():
        ...
        print("Ferdig med beregning av saldo, skriver tilbake ..."); NL(1)
        ...
        print("Fjerner bereregnet saldo for tomme rader fra radnummer", 9,"til radnummer", siste_tomme_rad-1, "..."); NL(1)
        for row_number in range(9, siste_tomme_rad):
        ...
        print("Ferdig med beregning og tilbake skriving av saldo etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)
...
def skriv_transaksjonstyper():
        ...
        print("-----------------------------------------"); NL(1)
        s_verdier = {s_celle[0] for s_celle in arbeidsark.iter_rows(min_row=3, max_row=99, min_col=(20+antall_nye_kolonner), max_col=(20+antall_nye_kolonner), values_only=True) if s_celle[0] is not None}
        t_verdier = {t_celle[0] for t_celle in arbeidsark.iter_rows(min_row=3, max_row=99, min_col=(21+antall_nye_kolonner), max_col=(21+antall_nye_kolonner), values_only=True) if t_celle[0] is not None}
        u_verdier = {u_celle[0] for u_celle in arbeidsark.iter_rows(min_row=3, max_row=99, min_col=(22+antall_nye_kolonner), max_col=(22+antall_nye_kolonner), values_only=True) if u_celle[0] is not None}

        print("Inntekter er:")
        print(s_verdier); NL(1); time.sleep(1)
        print("Utgifter er:")
        print(t_verdier); NL(1); time.sleep(1)
        print("Sparing er:") 
        print(u_verdier); NL(1); time.sleep(1)
        ...
        for rad in arbeidsark.iter_rows(min_row=9, min_col=4, max_col=7):
            ...
            if isinstance(celle_verdi, str) and celle_verdi.strip():
                ...
                break
        print("Ferdig med skriving av transaksjonstyper etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)
...
def skriv_endringstid():
        ...
        print("Endringstid er registrert som:", endringsdato); NL(3)
        arbeidsark.cell(row=6, column=4).value = arbeidsark.cell(row=9, column=6).value


def lagre():
    
        def synkroniser_til_OneDrive():    
        ...
        print("Ny data fra CSV-filer i mappen:", path_til_CSV, "er lagret i Excel-filen:", Excel_filnavn, "etter:", round(perf_counter() - start_tid, 3), "sekunder"); NL(3)
        print("----------------------------------------------------------------------------------------------------------------------------------")
        print("---------------------------------------------- Kode fullført etter", round(perf_counter() - total_tid_variabel, 3), "sekunder ----------------------------------------------")
        print("----------------------------------------------------------------------------------------------------------------------------------")
        NL(30)
...
def printStartInfo():
    ...
    # telle_variabel += 1
    print("----------------------------------------------------------------------------------------------------------------------------------")
    print("---------------------------------------------------- Starter kjøring nummer", telle_variabel, "----------------------------------------------------")
    print("----------------------------------------------------------------------------------------------------------------------------------"); NL(2)
    ...
    if osenv == 'posix':
        print("Registrert operativsystem er Linux"); NL(1)
        
    
    print("H-katalog er:", H_katalog); NL(1)
    
    with open(H_katalog + 'client_ID.txt', 'r') as file:
        CLIENT_ID = file.read().strip()
        print("CLIENT_ID er:", CLIENT_ID)
        if type(CLIENT_ID) != str or len(CLIENT_ID) == 0:
            print("CLIENT_ID er ikke regisrert som streng, avslutter"); NL(3)
            exit()
    ...
    with open(H_katalog + 'client_secret.txt', 'r') as file:
        ...
        if type(CLIENT_SECRET) != str or len(CLIENT_SECRET) == 0:
            print("CLIENT_SECRET er ikke registrert som streng, avslutter"); NL(3)
            exit()
    
    if type(CLIENT_SECRET) == str and type(CLIENT_ID) == str:
        NL(1);print("CLIENT_ID og CLIENT_SECRET er begge registrert som strenger, fortsetter ..."); NL(1)
        
        
    print("Path til CSV er:", path_til_CSV); NL(1)

    print("Hovedkatalog er:", hovedkatalog); NL(1)
    
    print("Excel-filnavn er:", Excel_filnavn); NL(1)
    
    path_til_Excel = os.path.normpath(os.path.join(hovedkatalog, Excel_filnavn))
    print("Excel-filbane er:", path_til_Excel); NL(1)
    ...
    print("Ekskluderer transaksjoner med disse beskrivelsene:", eksluder_disse_beskrivelser); NL(1)
    
    oppdater_fra_dato = (datetime.now() - timedelta(days=antall_dager_tilbake)).strftime("%Y-%m-%d")
    oppdater_til_dato = datetime.today().strftime('%Y-%m-%d')
    print("Oppdaterer data fra:", oppdater_fra_dato, "til:", oppdater_til_dato, "altså", antall_dager_tilbake, "dager tilbake"); NL(1)
    
    
    if pauseperiode_i_timer <= 1:
        print("Dato-kodet pauseperiode mellom hver kjøring er satt til:", pauseperiode_i_timer, "time"); NL(1)
    else:
        print("Dato-kodet pauseperiode mellom hver kjøring er satt til:", pauseperiode_i_timer, "timer"); NL(1)
        
    print("Startsaldo før tidligste transaksjon i regnearket er satt til:", startsaldo); NL(1)
    
    print("REFRESH_TOKEN_FILE er:", REFRESH_TOKEN_FILE); NL(1)
    print("ACCESS_TOKEN_FILE er:", ACCESS_TOKEN_FILE); NL(1)
    print("AccountKeyMap_Fil er:", AccountKeyMap_Fil); NL(1)
    
    
    time.sleep(3); NL(5); 
    print("----------------------------------------------------------------------------------------------------------------------------------")
    print("---------------------------------------------------- Starter program nummer", telle_variabel, "----------------------------------------------------")
    print("----------------------------------------------------------------------------------------------------------------------------------"); NL(2)
...
while True:
        ...
        # Konfigurering 2:
        path_til_CSV = '/home/andreas/Downloads/WindowsBackup-main/Arkiv/Økonomi/SB1_Data/'
        # path_til_CSV = 'C:/Users/andre/OneDrive/Arkiv/Økonomi/SB1_Data/'
        
        
        # Konfigurering 3:
        hovedkatalog = '/home/andreas/Downloads/WindowsBackup-main/Arkiv/Økonomi/'
        # hovedkatalog = 'C:/Users/andre/OneDrive/Arkiv/Økonomi/'

        
        # Konfigurering 4:
        Excel_filnavn = 'Budsjett.xlsx'
        arbeidsbok = last_arbeidsbok_global(); NL(2)
        ...
        # Konfigurering 6:    
        kontonummere = ['18131796605', '18133745544', '18135700327', '18227346139', '18138191555']
                
        
        # Konfigurering 7:
        eksluder_disse_beskrivelser = ['Jan Andreas Salvesen', 'Overføring', 'Nettbank til: 1813.37.45544']
    ...
    #Programstyring
        ...
        def Sparebank1():
            ...
            hent_transaksjonerSB1(); time.sleep(4)
            
            les_og_sorter(); time.sleep(4)

            # fjern_all_data_BtilG(); time.sleep(4)
            fjern_overlappende_data_BtilG(); time.sleep(4)
            
            les_og_sorter(); time.sleep(4)

            hent_kategoriser_og_skriv_CSV(); time.sleep(4)
            
            les_og_sorter(); time.sleep(4)
            
            skriv_predefinerte_transaksjoner(); time.sleep(4)

            beregn_og_skriv_saldo(); time.sleep(4)

            skriv_transaksjonstyper(); time.sleep(4)

            skriv_endringstid(); time.sleep(4)
        ...
        def DNB():
            print("DNB er ikke implementert enda"); NL(1)
            exit()
        def Nordea():
            print("Nordea er ikke implementert enda"); NL(1)
            exit()
        def DanskeBank():
            print("Danske Bank er ikke implementert enda"); NL(1)
            exit()
        def Handelsbanken():
            print("Handelsbanken er ikke implementert enda"); NL(1)
            exit()
        def Sbanken():
            print("Sbanken er ikke implementert enda"); NL(1)
            exit()
